...
public class Events<T extends Event> implements Listener, EventExecutor {
    ...
    public static <T extends Event> Events<T> subscribe(Class<T> clazz, EventPriority priority, boolean ignoreCancelled, Function<T, Boolean> func) {
        ...
        if (Bukkit.isPrimaryThread()) {
            Bukkit.getPluginManager().registerEvent(clazz, factory, priority, factory, ItemManage.getInstance(), ignoreCancelled);
        } else {
            Bukkit.getScheduler().runTask(ItemManage.getInstance(), () -> Bukkit.getPluginManager().registerEvent(clazz, factory, priority, factory, ItemManage.getInstance(), ignoreCancelled));
        }
        return factory;
    }
    ...
    public void unregister() {
        ...
        try {
            ...
            if (Bukkit.isPrimaryThread()) {
                ...
                Bukkit.getScheduler().runTask(ItemManage.getInstance(), () -> handlerList.unregister(this));
            }
